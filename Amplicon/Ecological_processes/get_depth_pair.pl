...
open IN, "bNTI_RCbray_depth.txt" || die;
open OUT, ">bNTI_RCbray_pair_depth.txt" || die;
<IN>;
while (<IN>) {
	chomp;
	#$_=~/(([A-Z])\.*\d+)\t(([A-Z])\.*\d+)\t(\S+)\t(\S+)/;
...
open IN, "bNTI_RCbray_pair_depth.txt" || die;
open OUT, ">bNTI_RCbray_pair_depth.stats" || die;
open OUTF, ">bNTI_RCbray_pair.stats_depth.list" || die;
